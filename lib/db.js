	...
	return new Promise((resolve, reject) => {
		...
		pool.getConnection((error, connection) => {
			...
			if(error) {
				...
				connection.query('SELECT type,SUM(type) as total FROM operations GROUP BY type', (error, rows) => {
					if(error) {
						return reject(error);
					}

					const data = {
							entries: rows[0].total,
	...
	return new Promise((resolve, reject) => {
		pool.getConnection((error, connection) => {
			...
			if(error) {
				...
				connection.query('SELECT * FROM operations ORDER BY id DESC LIMIT 10', (error, rows) => {
